...
1. [Parser Configuration](references/_sections.md#1-parser-configuration) — **CRITICAL**
   ...
   - 1.4 [Use parseAsHex for Color Values](references/parser-hex-colors.md) — MEDIUM (shorter URLs for color parameters)
...
2. [Adapter & Setup](references/_sections.md#2-adapter-&-setup) — **CRITICAL**
   ...
   - 2.3 [Verify Framework Compatibility Before Setup](references/setup-nextjs-version.md) — CRITICAL (prevents cryptic runtime errors from version mismatch)
   ...
   - 2.5 [Wrap App with NuqsAdapter](references/setup-nuqs-adapter.md) — CRITICAL (prevents hook failures from a missing provider)
...
4. [Server Integration](references/_sections.md#4-server-integration) — **HIGH**
   ...
   - 4.3 [Integrate useTransition for Loading States](references/server-use-transition.md) — HIGH (full visibility into server fetch pending state)
...
5. [Performance Optimization](references/_sections.md#5-performance-optimization) — **MEDIUM**
   - 5.1 [Debounce Server-Bound Search Updates](references/perf-debounce-search.md) — MEDIUM (reduces server requests during typing)
   ...
   - 5.4 [Use clearOnDefault for Clean URLs](references/perf-clear-on-default.md) — MEDIUM (reduces URL length for default values)
...
7. [Debugging & Testing](references/_sections.md#7-debugging-&-testing) — **LOW-MEDIUM**
   - 7.1 [Diagnose Common nuqs Errors](references/debug-common-errors.md) — LOW-MEDIUM
   - 7.2 [Enable Debug Logging for Troubleshooting](references/debug-enable-logging.md) — LOW-MEDIUM
   - 7.3 [Test Components with URL State](references/debug-testing.md) — LOW-MEDIUM
8. [Advanced Patterns](references/_sections.md#8-advanced-patterns) — **LOW**
   ...
   - 8.4 [Use urlKeys for Shorter URLs](references/advanced-url-keys.md) — LOW (reduces URL length for verbose params)
