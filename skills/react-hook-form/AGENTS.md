...
1. [Form Configuration](references/_sections.md#1-form-configuration) — **CRITICAL**
   ...
   - 1.3 [Choose shouldUnregister from Submission Semantics](references/formcfg-should-unregister.md) — HIGH (reduces memory usage for forms with frequently mounted/unmounted fields)
   - 1.4 [Choose Post-Submit Revalidation Deliberately](references/formcfg-revalidate-mode.md) — CRITICAL (reduces re-renders after initial submission)
   ...
   - 1.6 [Choose Validation Mode from UX and Validation Cost](references/formcfg-validation-mode.md) — CRITICAL (prevents re-renders on every keystroke)
2. [Field Subscription](references/_sections.md#2-field-subscription) — **CRITICAL**
   ...
   - 2.3 [Provide defaultValue to useWatch for Initial Render](references/sub-usewatch-default-value.md) — MEDIUM-HIGH
   ...
   - 2.6 [Use useWatch Instead of watch for Isolated Re-renders](references/sub-usewatch-over-watch.md) — CRITICAL (reduces re-renders in complex forms with multiple watchers)
...
3. [Controlled Components](references/_sections.md#3-controlled-components) — **HIGH**
   ...
   - 3.2 [Combine Local State with useController for UI-Only State](references/ctrl-local-state-combination.md) — MEDIUM (reduces form re-renders when UI state changes don't affect form data)
   - 3.3 [Split Controlled Fields When Isolation Helps](references/ctrl-single-usecontroller-per-component.md) — MEDIUM-HIGH
   ...
   - 3.5 [Wire Controller Field Props Correctly for UI Libraries](references/ctrl-controller-field-props.md) — HIGH (prevents form binding bugs and silent failures in UI library integrations)
4. [Validation Patterns](references/_sections.md#4-validation-patterns) — **HIGH**
   - 4.1 [Access Errors via Optional Chaining or Lodash Get](references/valid-error-message-strategy.md) — MEDIUM-HIGH
...
5. [Field Arrays](references/_sections.md#5-field-arrays) — **MEDIUM-HIGH**
   ...
   - 5.3 [Use field.id as Key in useFieldArray Maps](references/array-use-field-id-as-key.md) — MEDIUM-HIGH
   ...
   - 5.5 [Use Single useFieldArray Instance Per Field Name](references/array-unique-fieldarray-per-name.md) — MEDIUM-HIGH
6. [State Management](references/_sections.md#6-state-management) — **MEDIUM**
   - 6.1 [Align Submit Gating with Validation Mode](references/formstate-avoid-isvalid-with-onsubmit.md) — MEDIUM (prevents validation on every render for button disabled state)
...
7. [Integration Patterns](references/_sections.md#7-integration-patterns) — **MEDIUM**
   - 7.1 [Transform Values at Controller Level for Type Coercion](references/integ-value-transform.md) — MEDIUM (prevents type coercion bugs in numeric and date form fields)
...
8. [Advanced Patterns](references/_sections.md#8-advanced-patterns) — **LOW**
   ...
   - 8.3 [Memoize FormProvider Children Only After Profiling](references/adv-formprovider-memo.md) — LOW (prevents cascade re-renders from FormProvider state updates)
